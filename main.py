...
def box(kind, ALL, num):
    ...
    rubbishClass = "【" + str(ALL) + "】【" + kind + "】【" + str(num) + "】 OK!"
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            print(full)
            if "1" in full:
                fp = Process(target=fullbox, args=("有害垃圾",))
                fp.start()
            elif "2" in full:
                fp = Process(target=fullbox, args=("可回收垃圾",))
                fp.start()
            elif "3" in full:
                fp = Process(target=fullbox, args=("厨余垃圾",))
                fp.start()
            elif "4" in full:
                fp = Process(target=fullbox, args=("其他垃圾",))
                fp.start()
            if len(det):
                cycle = 0
                ...
                # Print results
                # 如果识别到的物体物体与上次结果一致，则计数加一；否则计数清零
